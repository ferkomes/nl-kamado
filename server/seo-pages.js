...
function seoPageHtml(html, path, lang) {
  ...
  if(product) {
    ...
    for (const [id,value] of Object.entries(product.type==='kamado'?{activeModelName:name,activeModelDesc:product.desc[lang],specPeople:product.people[lang]}:{detailAccessoryDesc:product.desc[lang]})) {
      body=body.replace(new RegExp('(<(p|h3|span|div)[^>]*id="'+id+'"[^>]*>)[\\s\\S]*?</\\2>'),(_,start,tag)=>start+seoEscape(value)+'</'+tag+'>');
    }
